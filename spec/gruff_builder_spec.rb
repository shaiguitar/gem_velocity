...
describe GruffBuilder do
  ...
  it "raises if no root" do
    lambda { builder = GruffBuilder.new(nil,nil,nil,nil, {}) }.should raise_error
  end

  it "has defaults" do
    builder = GruffBuilder.new(@tmp_dir,nil,nil,nil, {})
  ...
  it "can write out a file with gruff data" do
    gruff_options = {
      ...
      :line_data => [1,2,3,10,8,12],
      :min_value => 0,
      :max_value => 20,
    }
    builder = GruffBuilder.new(@tmp_dir,nil,"0.5.8","foo-baz", gruff_options)
    builder.write
    file = builder.absolute_filename
    File.exists?(file).should be_true
  end
